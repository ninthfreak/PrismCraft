...
- Cells still carry six colour slots, filled with a single flat constant. Nothing
  reaches the exporter, so this is cleanup rather than correctness — but the
  Paint / Bucket / Eyedropper tools and the palette still exist and now affect
  only the viewport. Shrinking the cell format will break existing `.res` saves
  unless `VoxelDefinition` gains a stride shim.
